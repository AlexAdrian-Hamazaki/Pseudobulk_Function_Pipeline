...
def getProportionCellsToSample(cell_proportion_info:list, variance_factor:float, totalSampleSize:int) -> float:
    ...
    proportion_to_sample = np.random.normal(baseline_cell_proportion, float(variance_factor)*3*float(stdev_cell_proportion), size = 1)
...
def scaleDictTo(dictNumberToSample: dict, totalSampleSize: int):
    ...
    if total_sum == 0:
        raise ValueError("Cannot scale dictionary with total sum of 0")

    # Step 1: scale
    scaled = {k: totalSampleSize * (v / total_sum) for k, v in dictNumberToSample.items()}

    # Step 2: floor
    floored = {k: int(np.floor(v)) for k, v in scaled.items()}

    # Step 3: compute remainders
    remainders = {k: scaled[k] - floored[k] for k in scaled}

    # Step 4: determine remaining cells
    remaining = totalSampleSize - sum(floored.values())

    # Only consider keys that had positive proportions
    valid_keys = [k for k, v in dictNumberToSample.items() if v > 0]

    # Sort by largest remainder
    sorted_keys = sorted(valid_keys, key=lambda k: remainders[k], reverse=True)

    # Step 5: distribute remaining cells
...
def getNumberToSample(dictCellTypeProportion:dict, totalSampleSize:int) -> dict:
    ...
    # if "Excitatory neurons" in scaled_dictNumberToSample: TODO Do the scaling better maybe
    #     scaled_dictNumberToSample["Excitatory neurons"] = 600
